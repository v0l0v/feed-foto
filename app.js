...
function render(entries) {
  ...
  el.innerHTML = entries.slice(0, 100).map(e => {
    ...
    return `<div class="card" data-color="?" data-id="${e._id}" data-source="${e._source}" onclick="openModal(this)">
      ...
      <div class="card-info">
        <div class="card-source">${e._source === 'lomography' ? 'Lomography Magazine' : e._source === 'booooooom' ? 'Booooooom' : e._source === 'tpj' ? 'The Photographic Journal' : e._source === 'swan' ? 'Swann Galleries' : e._source === 'huck' ? 'Huck Magazine' : 'Colossal · Fotografía'}</div>
